...
app.config['MONGO_URI'] = "mongodb+srv://dbUser:dbpassword@cluster0.v3meaob.mongodb.net/test"
mongo = PyMongo(app)
dbJD = mongo.db.JD
...
def uploadJD():
    try:
        ...
        if file and allowedExtension(file.filename):
            ...
            fetchedData = jdextractorObj.extractorData(
                "static/JD/"+filename, file.filename.rsplit('.', 1)[1].lower())
            result = None
            result1 = dbJD.insert_one({"Skills": list(
                fetchedData[0]), "Education": fetchedData[1], "JD_Data": fetchedData[2]}).inserted_id
            if result1 == None:
...
def showCandidates():
    ...
    if TopEmployeers == None:
        ...
        for i in TopEmployeers:
            selectedResumes[cnt] = {"Name": i['Name'], "Email": i['Email'], "Mobile_no": i['Mobile_no'], "Skills": i['Skills'],
                                    "Skills_percentage": i['Skills_percentage'], "Education": i['Education'], "Total_Percentage": i['Total_Percentage']}
            cnt += 1
...
def scanResume():
    ...
    for entry in entries:
        try:
            ...
            da = round((len(list(set(se['Skills']).intersection(
                data[3])))/len(list(se['Skills'])))*100, 3)
            skills_percentage = round((da * 60 / 100), 3)
            resume_percentage = matcher(se['JD_Data'], data[5])
            total = round(
                (skills_percentage + (resume_percentage * 40) / 100), 3)
            result = None
